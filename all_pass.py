...
class Pass_gen(ctk.CTk):
    def __init__(self):
        ...
        save_image_p = resource_path("saved_password_icon.png")
        #adding images from local directroy
        adding_save_image = ctk.CTkImage(Image.open(save_image_p).resize((23, 24), Image.LANCZOS))
        exit_image_p = resource_path("exit_icon.png")
        ...
        adding_exit_image = ctk.CTkImage(Image.open(exit_image_p).resize((23, 24), Image.LANCZOS))
        copy_button_p = resource_path("icon_for_copy.png")
        adding_copy_button = ctk.CTkImage(Image.open(copy_button_p).resize((23, 24), Image.LANCZOS))  
        insert_button_p = resource_path("insert_icon.png")
        adding_insert_button = ctk.CTkImage(Image.open(insert_button_p).resize((23, 24), Image.LANCZOS))
        generate_button_p = resource_path("generate_button.png")
        adding_generate_button = ctk.CTkImage(Image.open(generate_button_p).resize((23, 24), Image.LANCZOS))
        enter_button_p = resource_path("enter_icon.png")
        adding_enter_button = ctk.CTkImage(Image.open(enter_button_p).resize((23, 24), Image.LANCZOS))
        delete_button_p =resource_path("delete_selected.png")
        adding_delete_selected = ctk.CTkImage(Image.open(delete_button_p).resize((23, 24), Image.LANCZOS))
        delete_all_p = resource_path("delete_all.png")
        adding_delete_all = ctk.CTkImage(Image.open(delete_all_p).resize((23, 24), Image.LANCZOS))
        ...
        self.check_pin = ''
        self.save_pin = ''
        self.single_pin = ctk.CTkToplevel(fg_color=black)
        self.single_pin.geometry('500x200+400+340')

        self.single_pin.title("Making Pin")
       ...
       #log.configure(font=font_button)
        ...
        close = ctk.CTkButton(master=self.menu_frame,
                             ...
                             fg_color=button_colors)
        close.place(relx=0.01, rely=0.81)
        ...
        self.own_paswd = ctk.CTkButton(master=self.info_frame,
                             text='Add password',
                             anchor='m',
                             width=87,
                             height=32,
                             fg_color=button_colors,
                             hover_color=button_hover_color,
                             command=self.window_add_pass,
                             image=adding_insert_button)
        self.own_paswd.place(relx=0.5, rely=0.2, anchor='center')
        self.own_paswd.configure(font=font_button)
        #LABELS--
        ...
        #Tile of the password window
        self.password_title = ctk.CTkLabel(self.log_out,
                                           text='All Pass',
                                           fg_color='transparent')
        self.password_title.place(relx=0.5, rely=0.04, anchor='center')
        self.password_title.configure(font=logo_font)
        ...
        #entry
        self.website_entry = ctk.CTkEntry(self.info_frame, placeholder_text='Website', height=32, width=200, takefocus=True)
        ...
        self.username_entry = ctk.CTkEntry(self.info_frame, placeholder_text='Username', height=32, width=200, takefocus=True)
        ...
        self.username_entry.bind('<Left>', self.web_focus)
        self.website_entry.bind('<Right>', self.user_focus)


        #Search button on second frame
        self.search_entry = ctk.CTkEntry(self.saves_frame, placeholder_text='Search', height=32,width=200, takefocus=True)
        ...
        self.saved_info.bind('<Button-1>', self.remove_select)
    ...
    def add_to_line(self):
        if self.entry_inside_win.get() != '':
            self.password_name_coloumn.configure(text=self.entry_inside_win)
            #self.window.forget()
        else:
            pass
    ...
    def remove_select(self, event):
        for items in self.saved_info.selection():
            self.saved_info.selection_remove(items)
    ...
    def pin_is_right(self, event):
        if self.pin_entry.get() != pin:
            ...
            self.wrong_pin.place(relx=0.5, rely=0.45, relwidth=0.5, relheight=0.35, anchor='center')
            self.clear_entry()
        else:
            time.sleep(0.15)
    ...
    def search(self, event):
        a = self.search_entry.get().lower()
        make_it_str = ''
        t = []
        
        for items in self.saved_info.get_children():
            make_it_str = ''.join(str(self.saved_info.item(items)['values'][0:2]).lower())
            if a in make_it_str:
    ...
    def user_focus(self, event):
        self.username_entry.focus_set()

    def web_focus(self,event):
    ...
    #--------------------------------------------------------------------------------------------------------
    def window_add_pass(self):
        window = ctk.CTkToplevel(fg_color=black)
        window.geometry('450x150+400+340')

        window.title("Add Password")
        window.attributes("-topmost", True)
        #window.configure(bg='red')

        def add_to_line(event):
            if entry_inside_win.get() != '':
                self.password_name_coloumn.configure(text=entry_inside_win.get()[0:20])
                window.destroy()
        
        entry_inside_win = ctk.CTkEntry(window,
                                        placeholder_text='Add Password',
                                        width=250,
                                        height=34)
        entry_inside_win.bind("<Return>", add_to_line)

        if len(entry_inside_win.get()) < 150:
                self.pin_entry.delete(0, last_index=150)

        entry_inside_win.place(relx=0.5, rely=0.25, anchor='center')
        entry_inside_win.focus_set()
        entry_inside_win.configure(font=entry_font)

        top_win_button = ctk.CTkButton(window,
                               text='Add',
                               width=60,
                               height=40,
                               fg_color=button_colors,
        ...
        top_win_button.place(relx=0.6, rely=0.84, anchor='center')
        top_win_button.bind("<Button-1>", add_to_line)
        
        def close_win():
            window.destroy()
        
        win_button2 = ctk.CTkButton(window,
                               text='Cancel',
                               width=60,
                               height=40,
                               fg_color=button_colors,
        ...
        win_button2.place(relx=0.43, rely=0.84, anchor='center')
        window.resizable(False,False)
    #win_button2.bind('<Button-1>', self.cancel)
